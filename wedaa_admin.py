...
async def add_customer_to_sql(customer_data: dict):
    """
    Adds a new customer to the SQL database.
    Expects customer_data to have at least: name, phone_number, address.
    """
    session = SessionLocal()
    try:
        new_customer = Customers(
            ...
            google_maps_link=customer_data.get("google_maps_link", None)
        )
        session.add(new_customer)
        session.commit()
        session.refresh(new_customer)
        print(f"[INFO] Added new customer: {new_customer.name} ({new_customer.phone_number})")
        return new_customer
    except Exception as e:
        session.rollback()
...
async def process_order_pipeline(order: Order, user_input: str, llm):
    ...
    try:
        print("[INFO] Checking customer existence in SQL")
        session = SessionLocal()
        customer = session.query(Customers).filter_by(phone_number=completed_order.phone_number).first()
        session.close()
        if not customer:
            print("[INFO] Customer not found, adding new customer to SQL")
            customer_data = {
                "name": completed_order.customer_name,
                "phone_number": completed_order.phone_number,
                "address": completed_order.address,
                "google_maps_link": getattr(completed_order, "maps_link", None)
            }
            added_customer = await add_customer_to_sql(customer_data)
            if added_customer:
                completed_order.customer_id = added_customer.id
            else:
                print("[ERROR] Could not add customer to SQL")
        else:
            completed_order.customer_id = customer.id
            print(f"[INFO] Customer found in SQL: {customer.name} (ID: {customer.id})")
    except Exception as e:
        print(f"[ERROR] Customer verification/creation failed: {e}")
        return {"error": f"customer verification/creation failed: {e}"}

    # Optionally, update the JSON object with the latest customer info
    try:
        print("[INFO] Pulling latest customer info from SQL")
        session = SessionLocal()
        customer = session.query(Customers).filter_by(phone_number=completed_order.phone_number).first()
        session.close()
        if customer:
            completed_order.customer_name = customer.name
            completed_order.address = customer.address
            completed_order.customer_id = customer.id
            if hasattr(completed_order, "maps_link"):
                completed_order.maps_link = getattr(customer, "google_maps_link", None)
    except Exception as e:
        print(f"[ERROR] Failed to update order with latest customer info: {e}")
...
async def add_prices_to_mongo(prices: list):
    """
    Adds a list of price dictionaries to the prices collection in MongoDB.
    Each price should be a dict with keys: item_id, name, unit, price_per_unit.
    """
    from mongo import mongo_prices_collection
    try:
        if not prices:
            print("[INFO] No prices to add.")
            return
        # Remove all existing prices (optional, comment out if not needed)
        await asyncio.to_thread(mongo_prices_collection.delete_many, {})
        # Insert new prices
        await asyncio.to_thread(mongo_prices_collection.insert_many, prices)
        print(f"[INFO] Added {len(prices)} prices to MongoDB.")
    except Exception as e:
        print(f"[ERROR] Failed to add prices to MongoDB: {e}")

# Example usage:
async def main():
    empty_order = Order(phone_number="9494758674")
    user_input = "Sireesha srilanka number:9494758674 Peanut oil-1 Kusuma oil-1 Coconut oil-1 Address: a306, surya towers, izzath nagar, kondapur -500084"
